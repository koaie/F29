...
\begin{document} % All begin commands must be paired with an end command somewhere
    ...
    \section{$L=\{a^mb^na^m|m,n \geq 0\}$, alphabet = $\{a,b,Z_0\}$, Start stack symbol = $Z_0$}
        ...
        \end{tikzpicture}
    \section{Not possible as DFA express regular languages, and $a^m b^n a^m$ is a only expressable as a context-free language.}
    \section{}
    \section{}
    \section{$L=\{w|\#_a = \#_b\}$, alphabet = $\{a,b,Z_0\}$, Start stack symbol = $Z_0$}
        \begin{tikzpicture} [draw=cyan!70!black,
        node distance = 2cm, 
        on grid, 
        auto]
        ...
        \path [-stealth, thick]
            ...
            (sa) edge[bend left] node {$b,a,\epsilon$}   (sb)
            (sb) edge[bend left] node {$\epsilon,Z_0,\epsilon$}   (sf)

            (sa) edge [loop above]  node {$a,\epsilon,a$}()
            (sb) edge [loop above]  node {$b,a,\epsilon$}();
    \end{tikzpicture}
    \section{}
    \section{}
